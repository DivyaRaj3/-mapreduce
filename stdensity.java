...
public class stdensity {
  ...
  public static class IntSumReducer
    ...
    public void reduce(Text key, Iterable<IntWritable> values,
     ...
     int count = 0; // word count
      float density=0;// word count/total words * 100
      for (IntWritable val : values)
      {
        count += val.get();
      }
      density=(count/1000)*100;
     // result.set(count);
     // String s=Float.toString(count);
    //String st=Float.toString(density);
      String str=count+"\t"+density;
      int sts=Integer.parseInt(str);
      result.set(sts);
  ...
  public static void main(String[] args) throws Exception {
    ...
    job.setJarByClass(stdensity.class);
